...
class RN(nn.Module):
    ...
    def forward(self, x, reps=False):
        bsz = x.size(0)
        if 'imagenet' in args.dataname:
            inp = x.view(bsz, 3, 64, 64)
        else:
            inp = x.view(bsz, 3, 32, 32)

        out = relu(self.bn1(self.conv1(inp)))
        out = self.layer1(out)
        out = self.layer2(out)
        out = self.layer3(out)
        out = self.layer4(out)
...
def GEMResNet18(num_classes):
    return RN(BasicBlock, [2, 2, 2, 2], num_classes, nf=int(args.width_mult * 20))